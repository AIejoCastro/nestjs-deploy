...
describe('LoansService', () => {
  ...
  it('should create a loan when copy is available and no reservation exists', async () => {
    ...
    mockReservationsService.findPendingReservationByCopy.mockResolvedValue(
      null,
    );
    ...
    expect(mockLoansRepo.count).toHaveBeenCalledWith({
      where: { userId: 'user-1', status: LoanStatus.ACTIVE },
    });
  ...
  it('should throw if copy is reserved by another user', async () => {
    ...
    await expect(
      service.create('user-1', { copyId: 'copy-1' }),
  ...
  it('remove should delete returned loan without freeing copy', async () => {
    ...
    expect(mockCopiesService.updateStatus).not.toHaveBeenCalledWith(
      'copy-11',
      expect.any(String),
    );
